...
{{
  ...
  "confidence_score": 0.X
    ...
    with governance_context(agent_id="confidence_evaluator", run_id=root_task_id or "default-run"):
        messages = [
            SystemMessage(content=evaluation_prompt),
            HumanMessage(content=f"[RUN_ID: {root_task_id or 'unknown'}][AGENT: confidence_evaluator]\nEvaluate the candidate output against the user instruction and providing the required JSON response.")
        ]
        try:
